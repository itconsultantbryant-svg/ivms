...
export default function AddPurchaseDetails({
  ...
  const addSale = () => {
    ...
    const userID = Number(authContext.user) || parseInt(authContext.user, 10);
    ...
    fetch(`${API_BASE}/purchase/add`, {
      ...
      .then(({ ok, data }) => {
        if (ok) {
          const productName = (products.find((p) => p._id === data.productID || p.id === data.productID) || {}).name || "";
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg overflow-y-scroll">
                {showReceipt && lastPurchase ? (
                  <div className="bg-white px-4 pt-5 pb-4 sm:p-6">
                    <p className="text-sm font-medium text-green-600 mb-2">Purchase added. Print or download your receipt.</p>
                    ...
                    <button type="button" onClick={closeReceiptAndModal} className="mt-4 w-full rounded-md bg-gray-800 text-white py-2 text-sm font-medium hover:bg-gray-700">Done</button>
                  </div>
                ) : (
                ...
                <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  <button type="button" className="inline-flex w-full justify-center rounded-md bg-blue-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-500 sm:ml-3 sm:w-auto" onClick={addSale}>Add</button>
                  <button type="button" className="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto" onClick={() => addSaleModalSetting()} ref={cancelButtonRef}>Cancel</button>
                </div>
                </div>
                )}
              </Dialog.Panel>
